...
var _yAxis=d3.axisLeft(y).ticks(this.axisData.y.niceTicks||6);console.log(this.axisData.y.tickFormat);if(this.axisData.y.tickFormat){_yAxis.tickFormat(d3.format(this.axisData.y.tickFormat))}svg.append('g').attr('class','y-axis').call(_yAxis);if(this.axisData.y.axisLabel){svg.append('text').attr('transform','rotate(-90)').attr('y',0-margin.left).attr('x',0-height/2).attr('dy','1em').attr('class','y-axis-label').text(this.axisData.y.axisLabel)}if(this.axisData.x.axisLabel){svg.append('text').attr('dy','1em').attr('class','x-axis-label').attr('text-anchor','middle').attr('transform','translate('+width/2+','+(height+margin.top)+')').text(this.axisData.x.axisLabel)}this.fire('chart-drawn',{})},_redraw:function _redraw(newData,oldData){if(oldData&&oldData.length){Polymer.dom(this.$.chart).node.innerHTML='';this.draw()}},_toggleSeries:function _toggleSeries(event){var label='series-circle-'+event.model.get('idx');this[label]=!this[label];if(this[label]){this.querySelectorAll('.'+label).forEach(function(elt){elt.style.display='none'})}else{this.querySelectorAll('.'+label).forEach(function(elt){elt.style.display='block'})}}})})();
